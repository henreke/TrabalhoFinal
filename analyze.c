...
/* File: analyze.c                                  */
/* Semantic analyzer				    */
/*Arquivo baseado no analyze.c presente no livro    */
  ...
  switch (t->nodekind)
	...
	case AssignK:
	  ...
	  if(l == NULL){
	    ...
	    if (l->tnode_p->type != Integer) if (l->tnode_p->type != String) {
	  ...
	  if(r == NULL) {  //expr
	    ...
	    if (l->tnode_p->type ==String)
	   	 if (r->tnode_p->type != String) {
	      		typeError(t->child[1],"not string");
	    	 }
